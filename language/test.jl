...
three_slides_task = Task([Apparatus([Path(left), Path(right)]), Apparatus([Path(left)])], false)
...
four_cups_task = Task([Apparatus([Cup(blue), Cup(yellow)]), Apparatus([Cup(red), Cup(green, true)])], false)
